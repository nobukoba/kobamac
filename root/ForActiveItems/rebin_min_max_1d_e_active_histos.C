void rebin_minx_max_1d_e_active_histos(){
  ...
  for (Int_t i=0; i < arr->GetEntries(); i++){
    ...
    gROOT->ProcessLine(Form(".L %s/root/cui/rebin_minx_max_1d_e.C", gEnv->GetValue("KOBAMAC_DIR",".")));
    gROOT->ProcessLine(Form("rebin_minx_max_1d_e((TH1*)%p,%d)", hist,par0,par1,par2));
  }
  return;
}
